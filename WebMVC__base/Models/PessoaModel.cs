...
namespace _2020_05_12_WebMVC.Models
{
    public class PessoaModel
    {
        ...
        public DataTable Listar(string sortOrder, string searchString)
        {
            ...
            {
                sql += " WHERE PessoaNome like @PessoaNome ";
            }
            sql += " ORDER BY PessoaNome ";
            sql += (sortOrder == "nomeDesc") ? "DESC" : "";
            ...
            using (SqlConnection Conexao = new SqlConnection(connectionString))
            {
                ...
                if (searchString != null)
                {
                    da.SelectCommand.Parameters.AddWithValue("@PessoaNome", searchString);
                }
        ...
        // Método para excluir definitivamente o registro da tabela:
        public void Excluir(int id)
        {
            using (SqlConnection Conexao = new SqlConnection(connectionString))
            {
                // Comando CRUD - INSERT:
                using (SqlCommand Comando = new SqlCommand(
                    "DELETE FROM tb_Pessoa WHERE PessoaID = @PessoaID", Conexao))
                {
                    // Substituindo o atributo do comando DELETE pelo parâmetro passado no método:
                    Comando.Parameters.AddWithValue("@PessoaID", id);

                    Conexao.Open();             // Abrindo conexão com BD.
                    Comando.ExecuteNonQuery();  // Executando o comando DELETE.
                    Conexao.Close();            // Fechando conexção com BD:
                }
            }
        }
